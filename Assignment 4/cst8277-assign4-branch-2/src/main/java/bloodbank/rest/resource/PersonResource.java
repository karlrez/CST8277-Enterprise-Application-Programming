 ...
 * @author (original) Mike Norman update by : Group 41: dan brown, daniel dwyer, karl rezansoff
 */
...
public class PersonResource {
	...
	public Response addPhoneForPerson(@PathParam(RESOURCE_PATH_ID_ELEMENT) int id, Phone phone) {
		Response response = null;
		Person updatedPerson = service.setPhoneFor(id, phone);
		if (updatedPerson == null)
			return Response.status(Status.NOT_FOUND).build();
		response = Response.ok(updatedPerson).build();
		return response;
